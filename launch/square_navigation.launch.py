...
def generate_launch_description():
    ...
    square_path = os.path.join(get_package_share_directory('autonomous_tb3'),'world','square','model.sdf')
    config_dir = os.path.join(get_package_share_directory('autonomous_tb3'),'config')
    map_file = os.path.join(config_dir,'square.yaml')
    ...
    y_pose = LaunchConfiguration('y_pose', default='2.8')
    ...
    square_spawner = Node(
        ...
        name='square_spawner',
        arguments=[square_path,"b","0.0" ,"0.0" ]

    )

    square_localization = IncludeLaunchDescription(
    ...
    square_mapping = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(get_package_share_directory('slam_toolbox'), 'launch', 'online_async_launch.py')
        ),
    )

    square_nav_profiling = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(get_package_share_directory('autonomous_tb3'), 'launch', 'navigation_launch_profiling.launch.py')),
        launch_arguments={
        # 'map':map_file,
        'params_file': params_file,
        'use_sim_time': use_sim_time,
    ...
    square_nav = IncludeLaunchDescription(
